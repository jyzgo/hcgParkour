...
namespace MTUnity.Actions {
	public static class MTActionExtension  {
        ...
        public static MTActionState ForceRunAction(this GameObject target,MTAction action)
        {
            Debug.Assert(action != null, "Argument must be non-nil");
            return MTActionManager.Instance.AddAction(action, target, false);
        ...
        public static MTActionState ForceRunActions(this GameObject target, params MTFiniteTimeAction[] actions)
        {
            Debug.Assert(actions != null, "Argument must be non-nil");
            Debug.Assert(actions.Length > 0, "Paremeter: actions has length of zero. At least one action must be set to run.");
...
#pragma warning restore CS0618 // 类型或成员已过时
        ...
        public static void setColorWithOutOpacity(this Graphic target, Color color)
        {
            float a = target.color.a;
            target.color = new Color(color.r, color.g, color.b, a);

        }
        public static void setOpacity(this Graphic target,float curA)
        {
			if(target != null)
			{
				Color originalColor = target.color;
        ...
        public static void setOpacity(this SpriteRenderer target, float curA)
        {
			if(target != null)
			{
				Color originalColor = target.color;
